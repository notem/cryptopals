...
challenge5 = __import__("challenge5")
...
def probable_keysizes(binary, max_keysize):
    ...
    for key_size in range(2, max_keysize + 1):
        ...
        dif1 = hamming_distance(blocks[0], blocks[1]) + hamming_distance(blocks[1], blocks[2]) + hamming_distance(
            blocks[2], blocks[3])
        dif2 = hamming_distance(blocks[0], blocks[2]) + hamming_distance(blocks[0], blocks[3]) + hamming_distance(
            blocks[1], blocks[3])
        dif = dif1 + dif2 / 6
        dif /= key_size
